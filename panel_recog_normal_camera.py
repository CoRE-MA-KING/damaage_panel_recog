...
def setup_camera(device, width=1280, height=720, fps=30):
    """汎用Webカメラを初期化する."""
    ...
    # 可能なら自動露出 / 自動WBを切る
    try:
        # バックエンドによって値の意味が異なることがあります
        cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 1)  # 1: manual に相当することが多い
    except Exception:
        pass
    try:
    ...
    def safe_set(prop, val):
        try:
            cap.set(prop, float(val))
        except Exception as e:
            print(f"[WARN] set({prop}) failed: {e}")

    # 初期パラメータを設定（サポートされていない項目は無視される）
    safe_set(cv2.CAP_PROP_EXPOSURE,   RS_EXPOSURE)
    safe_set(cv2.CAP_PROP_GAIN,       RS_GAIN)
    safe_set(cv2.CAP_PROP_BRIGHTNESS, RS_BRIGHTNESS)
    safe_set(cv2.CAP_PROP_CONTRAST,   RS_CONTRAST)
    safe_set(cv2.CAP_PROP_SATURATION, RS_SATURATION)
    safe_set(cv2.CAP_PROP_SHARPNESS,  RS_SHARPNESS)
    safe_set(cv2.CAP_PROP_GAMMA,      RS_GAMMA)
    try:
        cap.set(cv2.CAP_PROP_WB_TEMPERATURE, RS_WHITEBALANCE)
    except Exception:
        pass
...
def main():
    ...
    if use_gui:
        ...
        init = dict(
            exposure   = max(1, cam_get(cv2.CAP_PROP_EXPOSURE,   RS_EXPOSURE)),
            gain       = cam_get(cv2.CAP_PROP_GAIN,              RS_GAIN),
            wb         = cam_get(cv2.CAP_PROP_WB_TEMPERATURE,    RS_WHITEBALANCE),
            brightness = cam_get(cv2.CAP_PROP_BRIGHTNESS,        RS_BRIGHTNESS),
            contrast   = cam_get(cv2.CAP_PROP_CONTRAST,          RS_CONTRAST),
            sharpness  = cam_get(cv2.CAP_PROP_SHARPNESS,         RS_SHARPNESS),
            saturation = cam_get(cv2.CAP_PROP_SATURATION,        RS_SATURATION),
            gamma      = cam_get(cv2.CAP_PROP_GAMMA,             RS_GAMMA),
        )

        # カメラパラメータ用トラックバー
        cv2.createTrackbar('Exposure',     MAIN_WIN, init["exposure"],   10000, cam_set(cv2.CAP_PROP_EXPOSURE))
        cv2.createTrackbar('Gain',         MAIN_WIN, init["gain"],        255, cam_set(cv2.CAP_PROP_GAIN))
        cv2.createTrackbar('WhiteBalance', MAIN_WIN, init["wb"],         10000, cam_set(cv2.CAP_PROP_WB_TEMPERATURE))
        cv2.createTrackbar('Brightness',   MAIN_WIN, init["brightness"],  255, cam_set(cv2.CAP_PROP_BRIGHTNESS))
        cv2.createTrackbar('Contrast',     MAIN_WIN, init["contrast"],    255, cam_set(cv2.CAP_PROP_CONTRAST))
        cv2.createTrackbar('Sharpness',    MAIN_WIN, init["sharpness"],   255, cam_set(cv2.CAP_PROP_SHARPNESS))
        cv2.createTrackbar('Saturation',   MAIN_WIN, init["saturation"],  255, cam_set(cv2.CAP_PROP_SATURATION))
        cv2.createTrackbar('Gamma',        MAIN_WIN, init["gamma"],      1000, cam_set(cv2.CAP_PROP_GAMMA))
            ...
            for c in ['blue', 'red']:
                ...
                for (top, bottom) in pairs:
                    ...
                    if not (use_track and _MOTPY_AVAILABLE):
                        center_x = w_img // 2
                        if abs(cx - center_x) < abs(target_to_center_distance):
                            target_to_center_distance = cx - center_x
